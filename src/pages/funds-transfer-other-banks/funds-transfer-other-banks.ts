...
export class FundsTransferOtherBanksPage {
	...
	ftToOtherBanksFormData = { destinationType: '', toWho: '', destinationBankAccount:'', destinationBank: '', 
		recAccountNo: '', recWalletNo: '', amount: '', narration: '', pin1: '', pin2: '', pin3: '', pin4: '' };
	...
	constructor(public platform: Platform,  public storage: Storage, public loadingCtrl: LoadingController, public alertCtrl: AlertController, public http: HttpClient, public actionSheetCtrl: ActionSheetController, public toastCtrl: ToastController, public afAuth: AngularFireAuth, public fb: FormBuilder, public navCtrl: NavController, public navParams: NavParams) {
		...
		this.destinationType = this.ftToOtherBanksForm.controls['destinationType'];
		this.toWho = this.ftToOtherBanksForm.controls['toWho'];
		this.destinationBankAccount = this.ftToOtherBanksForm.controls['destinationBankAccount'];
		this.destinationBank = this.ftToOtherBanksForm.controls['destinationBank'];
		this.recAccountNo = this.ftToOtherBanksForm.controls['recAccountNo'];
		this.recWalletNo = this.ftToOtherBanksForm.controls['recWalletNo'];
		this.amount = this.ftToOtherBanksForm.controls['amount'];
		this.narration = this.ftToOtherBanksForm.controls['narration'];
		this.pin1 = this.ftToOtherBanksForm.controls['pin1'];
		this.pin2 = this.ftToOtherBanksForm.controls['pin2'];
		this.pin3 = this.ftToOtherBanksForm.controls['pin3'];
		this.pin4 = this.ftToOtherBanksForm.controls['pin4'];
	}
	...
	onDestinationTypeChange()
	{
		...
		if(this.ftToOtherBanksFormData.destinationType=='Transfer To A Wallet')
		{
	...
	{
		if(this.ftToOtherBanksFormData.toWho=='Self')
		{
			this.displayAmount = true;
		}
		else
	...
	verifyAccountNo(recAccountNo)
	{
		this.storage.get('zambia_bank_customer_token').then((val) => {
			...
			var form_params = "";
			form_params = form_params + "&receipientBankNumber=" + encodeURI(this.ftToOtherBanksFormData.recAccountNo);
			form_params = form_params + "&bankId=" + encodeURI(this.ftToOtherBanksFormData.destinationBank);
	...
	verifyWalletNo(recWalletNo)
	{
		this.storage.get('zambia_bank_customer_token').then((val) => {
			...
			var form_params = "";
			form_params = form_params + "&receipientBankNumber=" + encodeURI(this.ftToOtherBanksFormData.recWalletNo);
	...
	doFundsTransfer(ftToOtherBanksFormData)
	{
		console.log(ftToOtherBanksFormData);
		this.storage.get('zambia_bank_customer_token').then((val) => {
			this.token = val;
			...
			var parameter = "";
			let url = "";
			if(this.ftToOtherBanksFormData.destinationType=='Transfer To A Bank')
			{
				if(this.ftToOtherBanksFormData.toWho=='Self')
				{
					var destinationDetails = this.ftToOtherBanksFormData.destinationBankAccount.split("###");
					url = "http://localhost:8080/EKwachaWebService/NCE/services/FundsTransferServices/fundsTransferEkwachaWalletToBank";
					var form_params = "";
					form_params = form_params + "&receipientAccountNumber=" + encodeURI(destinationDetails[1]);
					form_params = form_params + "&creditBankId=" + encodeURI(destinationDetails[2]);
					form_params = form_params + "&amount=" + encodeURI(this.ftToOtherBanksFormData.amount);
					form_params = form_params + "&selectTransferType=" + encodeURI('W2BS');
					form_params = form_params + "&orderRef=" + encodeURI(orderRef);
					form_params = form_params + "&pin=" + encodeURI((this.ftToOtherBanksFormData.pin1 + "" + this.ftToOtherBanksFormData.pin2 + "" + this.ftToOtherBanksFormData.pin3 + "" + this.ftToOtherBanksFormData.pin4));
					
					
					parameter = form_params;
				}
				else if(this.ftToOtherBanksFormData.toWho=='Outside')
				{
					url = "http://localhost:8080/EKwachaWebService/NCE/services/FundsTransferServices/fundsTransferEkwachaWalletToBank";
					var form_params = "";
					form_params = form_params + "&receipientAccountNumber=" + encodeURI(this.ftToOtherBanksFormData.recAccountNo);
					form_params = form_params + "&creditBankId=" + encodeURI(this.ftToOtherBanksFormData.destinationBank);
					form_params = form_params + "&amount=" + encodeURI(this.ftToOtherBanksFormData.amount);
					form_params = form_params + "&selectTransferType=" + encodeURI('W2BA');
					form_params = form_params + "&orderRef=" + encodeURI(orderRef);
					form_params = form_params + "&pin=" + encodeURI((this.ftToOtherBanksFormData.pin1 + "" + this.ftToOtherBanksFormData.pin2 + "" + this.ftToOtherBanksFormData.pin3 + "" + this.ftToOtherBanksFormData.pin4));
			...
			else if(this.ftToOtherBanksFormData.destinationType=='Transfer To A Wallet')
			{	
				url = "http://localhost:8080/EKwachaWebService/NCE/services/FundsTransferServices/fundsTransfeEkwachaWalletToEkwachaWallet";
				
				var form_params = "";
				form_params = form_params + "&crAccountId=" + encodeURI(this.receipientCustomerWalletId);
				form_params = form_params + "&amount=" + encodeURI(this.ftToOtherBanksFormData.amount);
				form_params = form_params + "&selectTransferType=" + encodeURI('W2WA');
				form_params = form_params + "&orderRef=" + encodeURI(orderRef);
				form_params = form_params + "&pin=" + encodeURI((this.ftToOtherBanksFormData.pin1 + "" + this.ftToOtherBanksFormData.pin2 + "" + this.ftToOtherBanksFormData.pin3 + "" + this.ftToOtherBanksFormData.pin4));
